      ...
      <div className="scrollbar-hide z-50 flex w-full overflow-x-auto md:hidden">
        <div className="flex min-w-fit items-center gap-2">
          {artPreferences?.map((pref, index) => (
            <button
              key={index}
              className={`motion-duration-500 motion-preset-expand font-bricolage group inline-flex w-fit flex-shrink-0 rounded-md border !border-[#666666] px-2 py-1 text-sm font-normal text-nowrap text-black transition-all duration-300 hover:scale-110 active:scale-95 lg:text-[16px] ${isItemSelected(pref) && "bg-[#1E3A8A] transition-all duration-200"} `}
              style={{
            ...
            >
              <p
                className={cn(
                  ...
                  isItemSelected(pref) && "text-white",
                )}
              >
                {pref}
              </p>
